...
from constants.transfer import kSpindexer, kTrasnfer
...
class TransferSubsystem(Subsystem):
    def __init__(self):
        ...
        self.up_transfer_motor.configurator.apply(kTrasnfer._CONFIG)
        ...
        self.up_transfer_nt.float("Target RPM", kTrasnfer.TARGET_RPM)
        
        self.spindex_editable_pid = EditablePID("Transfer/Spindexer", self.spindex_motor, kSpindexer._CONFIG)
        self.up_transfer_editable_pid = EditablePID("Transfer/Up Transfer", self.up_transfer_motor, kTrasnfer._CONFIG)
    
    def activate(self):
        self.spindex_motor.set_control(self.spindex_velocity_voltage.with_velocity(kSpindexer.TARGET_RPM))
        self.up_transfer_motor.set_control(self.up_transfer_velocity_voltage.with_velocity(kTrasnfer.TARGET_RPM))
    ...
    def periodic(self):    
        ...
        kTrasnfer.TARGET_RPM = self.up_transfer_nt.get("Target RPM")
