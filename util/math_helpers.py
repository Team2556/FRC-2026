...
def deadband(inputValue: number, band: number) -> number:
    if abs(inputValue) <= band:
        return 0
    return inputValue
...
def clampRotation(
    inputValue: Rotation2d, minimum: Rotation2d, maximum: Rotation2d
) -> Rotation2d:
    return Rotation2d(clamp(inputValue.radians(), minimum.radians(), maximum.radians()))
